...
const unsigned long IMU_REPORT_INTERVAL_US = 10000;   // 100 Hz, nhanh hơn loop điều khiển
...
const float KP = 1.0, KI = 0.8, KD = 0.05;
  ...
  while (bno08x.getSensorEvent(&imu_value)) {
    switch (imu_value.sensorId) {
      ...
      case SH2_GYROSCOPE_CALIBRATED:
        // Chỉ dùng trục Z để thay thế wz tính từ encoder
...
void setMotorLR(uint8_t L, uint8_t R, float u) {
  int pwm = (int)(fabs(u) * 255.0 / RPM_MAX);
...
void kalmanPredictUpdate(KalmanD &k, float e_meas, float dt) {
  ...
  float dt3 = dt2 * dt;
  float dt4 = dt2 * dt2;
  float Q_pos = Q_var * dt4 / 4.0;
  float Q_vel = Q_var * dt2;
  float Q_cross = Q_var * dt3 / 2.0;
...
float PID_WHEEL_RPM(float rpm_ref, float rpm_meas, float &integral, float &ref_filter,
  ...
  kalmanPredictUpdate(k, err, dt);

  float d_term = KD * k.de_hat;
  float u_pre = KP * err + integral + d_term;

  if (fabs(rpm_ref) < 2.0 && fabs(rpm_meas) < 8.0) {
    integral *= 0.8;
  } else {
    integral += KI * err * dt;
  }
  ...
  float u = KP * err + integral + d_term;
  return constrain(u, -RPM_MAX, RPM_MAX);
}
...
void updateOdometry(float dt) {
  float v_fl = rpmToWheelMps(rpm[0]);
  float v_fr = rpmToWheelMps(rpm[1]);
  float v_bl = rpmToWheelMps(rpm[2]);
  float v_br = rpmToWheelMps(rpm[3]);
...
void computeTargetWheelRPM() {
  ...
  if (pos_err < POS_TOL_M && fabs(dtheta) < THETA_TOL_RAD && speed < SPEED_TOL_MPS) {
    target_hold_counter++;
  ...
  for (int i = 0; i < 4; i++) {
    target_rpm[i] = wheelMpsToRpm(wheel_mps[i] * scale);
  }
}
...
void loop() {
  ...
  float motor_cmd[4];
  for (int i = 0; i < 4; i++) {
    motor_cmd[i] = PID_WHEEL_RPM(target_rpm[i], rpm[i], I_term[i], filt_ref[i], kf[i], dt);
  }

  setMotorLR(M1_L, M1_R, motor_cmd[0]);
  setMotorLR(M2_L, M2_R, motor_cmd[1]);
  setMotorLR(M3_L, M3_R, motor_cmd[2]);
  setMotorLR(M4_L, M4_R, motor_cmd[3]);
