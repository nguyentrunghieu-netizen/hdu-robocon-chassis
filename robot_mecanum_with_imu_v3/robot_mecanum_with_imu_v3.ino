...
// [V3] Tham số Kalman 2-state cho RPM bánh xe
// State: [rpm, drpm_dt] - vận tốc và gia tốc của bánh
// Q_RPM: process noise variance (lớn hơn -> tin đo hơn, nhanh hơn nhưng ồn hơn)
// R_RPM: measurement noise variance (lớn hơn -> tin model hơn, mượt hơn nhưng trễ hơn)
// Tỉ số Q/R quyết định tốc độ phản ứng. Khởi đầu hợp lý: Q=50000, R=25 -> mượt, ít trễ
const float KALMAN_Q_RPM = 50000.0f;   // (RPM/s^2)^2, tune tăng nếu motor đáp ứng nhanh hơn estimate
const float KALMAN_R_RPM = 25.0f;      // (RPM)^2, tune theo nhiễu thực tế của encoder

const float WHEEL_KP_PWM_PER_RPM = 0.20f;
const float WHEEL_KI_PWM_PER_RPM_S = 0.75f;
const float WHEEL_KD_PWM_PER_RPM_S = 0.008f;  // [V3] D-term trên measurement, dùng drpm_hat từ Kalman
...
float rpm[4] = {0};            // RPM thô tính từ delta encoder (có nhiễu)
float target_rpm[4] = {0};
float I_term[4] = {0};
float filt_ref[4] = {0};

// [V3] Kalman filter 2-state cho mỗi bánh
// rpm_hat: ước lượng RPM (mượt, ít trễ hơn low-pass)
// drpm_hat: ước lượng gia tốc bánh (RPM/s) - dùng cho D-term trên measurement
struct KalmanRPM {
  float rpm_hat;       // estimate of RPM
  float drpm_hat;      // estimate of dRPM/dt (RPM per second)
...
// ==================== [V3] KALMAN FILTER CHO RPM ====================
// Mô hình động học:
//   rpm(k+1)      = rpm(k) + dt * drpm(k)         (gia tốc tích phân thành vận tốc)
//   drpm(k+1)     = drpm(k) + w                   (gia tốc là random walk)
//   F = [[1, dt],
//        [0,  1]]
// Đo lường:
//   y = rpm + v   (chỉ đo được RPM, không đo được gia tốc)
//   H = [1, 0]
//
...
void kalmanInitRPM(KalmanRPM &k) {
  k.rpm_hat = 0.0f;
  k.drpm_hat = 0.0f;
...
void kalmanUpdateRPM(KalmanRPM &k, float rpm_meas, float dt) {
  // ----- PREDICT -----
  // x_pred = F * x:  rpm_hat += dt * drpm_hat ; drpm_hat giữ nguyên
  k.rpm_hat += dt * k.drpm_hat;
  ...
  // Q ma trận quá trình (chuẩn cho mô hình tích phân với continuous-time variance Q_RPM)
  float Q11 = KALMAN_Q_RPM * dt4 / 4.0f;
  float Q12 = KALMAN_Q_RPM * dt3 / 2.0f;
  float Q22 = KALMAN_Q_RPM * dt2;

  float P00 = k.P[0][0] + dt * (k.P[0][1] + k.P[1][0]) + dt2 * k.P[1][1] + Q11;
  float P01 = k.P[0][1] + dt * k.P[1][1] + Q12;
  float P10 = k.P[1][0] + dt * k.P[1][1] + Q12;
  float P11 = k.P[1][1] + Q22;

  // ----- UPDATE -----
  // y = z - H*x_pred = rpm_meas - rpm_hat
  // S = H*P*H^T + R = P00 + R_RPM
  // K = P*H^T / S = [P00; P10] / S
  float y = rpm_meas - k.rpm_hat;
...
// Reset state Kalman về 0 (giữ P để filter "ấm sẵn" nếu motor chỉ tạm dừng)
void kalmanResetState(KalmanRPM &k) {
  k.rpm_hat = 0.0f;
  k.drpm_hat = 0.0f;
}
...
void stopMotion() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    I_term[i] = 0.0f;
    filt_ref[i] = 0.0f;
    kalmanResetState(kf[i]);   // [V3] Reset Kalman state khi dừng
  }
  hardStopAllMotors();
}
...
void resetOdometry() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    filt_ref[i] = 0.0;
    I_term[i] = 0.0;
    kalmanInitRPM(kf[i]);    // [V3] Init lại Kalman (cả state lẫn covariance)
  }
...
void hardStopAllMotors() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    I_term[i] = 0.0f;
    filt_ref[i] = 0.0f;
    kalmanResetState(kf[i]);   // [V3] Reset state Kalman, giữ P để filter "ấm sẵn"
  }
}
...
// ==================== PID TỪNG BÁNH ====================
// [V3] Cấu trúc PID = uFF + KS + KP*err + I + KD*(-drpm_hat)
//   uFF       : feedforward (RPM -> PWM danh nghĩa) -> phản ứng nhanh
//   KS        : bù ma sát tĩnh (chỉ khi có lệnh chạy)
//   KP*err    : proportional trên sai số
//   I         : tích phân (có conditional anti-windup)
//   KD*drpm   : DERIVATIVE TRÊN MEASUREMENT (không trên error)
//               -> Tránh "derivative kick" khi target nhảy bậc
//               -> drpm_hat đã được Kalman lọc, không cần lo nhiễu
//               -> Dấu âm vì khi bánh tăng tốc nhanh, ta giảm command để damping
float PID_WHEEL_RPM(float rpm_ref, float rpm_meas, float &integral, float &ref_filter,
                    float drpm_hat, float dt) {
  ...
  float d = -WHEEL_KD_PWM_PER_RPM_S * drpm_hat;   // [V3] D trên measurement
  float uPre = uFF + p + d + integral;
  ...
  uPre = uFF + p + d + integral;
  return constrain(uPre, -255.0f, 255.0f);
}

// ==================== ODOMETRY ====================
// [V3] Dùng Kalman 2-state thay low-pass đơn giản:
//   - rpm[i]       : RPM đo thô (có nhiễu)
//   - kf[i].rpm_hat: RPM đã lọc Kalman (mượt, dùng cho odometry và P-term)
//   - kf[i].drpm_hat: gia tốc bánh đã lọc (dùng cho D-term)
void updateMeasuredWheelRPM(float dt) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (abs(delta) > 10000) delta = 0;       // bảo vệ overflow / glitch
    rpm[i] = delta * ticks_to_rpm;           // RPM thô (chỉ để debug)
    kalmanUpdateRPM(kf[i], rpm[i], dt);      // [V3] Kalman estimate
...
void updateOdometry(float dt) {
  // [V3] Dùng Kalman estimate thay rpm_filt
  float v_fl = rpmToWheelMps(kf[0].rpm_hat);
  float v_fr = rpmToWheelMps(kf[1].rpm_hat);
  float v_bl = rpmToWheelMps(kf[2].rpm_hat);
  float v_br = rpmToWheelMps(kf[3].rpm_hat);
...
void setup() {
  ...
  // [V3] Init Kalman 2-state cho 4 bánh
  for (int i = 0; i < 4; i++) kalmanInitRPM(kf[i]);
  ...
  Serial.println("Coordinate control ready (+ IMU yaw fusion + Kalman wheel RPM) [V3]");
  Serial.println("CMD: SQ | G x_cm y_cm theta_deg | S | R");
...
void loop() {
  ...
  float motor_cmd[4];
  for (int i = 0; i < 4; i++) {
    // [V3] Dùng kf[i].rpm_hat làm rpm_meas, truyền drpm_hat cho D-term
    motor_cmd[i] = PID_WHEEL_RPM(target_rpm[i], kf[i].rpm_hat, I_term[i], filt_ref[i],
                                 kf[i].drpm_hat, dt);
  }
  ...
  for (int i = 0; i < 4; ++i) {
    if (fabs(kf[i].rpm_hat) > WHEEL_MEAS_ZERO_RPM) {   // [V3] Dùng Kalman estimate
      nearStopMeas = false;
      break;
    }
  }
