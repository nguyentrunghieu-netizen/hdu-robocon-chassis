...
Camera LiDAR Web V7
===================
Nang cap tu V6 voi 3 tinh nang moi:
  1. ENCODER FUSION  : blend measured_vx/wz tu Arduino vao ICP init-delta + deskewing
  2. A* PATH PLANNING: tim duong di tranh vat can tren occupancy grid
  3. OBSTACLE AVOIDANCE: phat hien vat can phia truoc, giam/dung truoc khi va cham

Giu nguyen:
  - Ball chaser (Kalman + PID + alignment auto-tuner)
  - LiDAR SLAM (KISS-ICP: deskewing + adaptive threshold + local map)
  - Persistent map (raycast occupancy) + relocalization
  - Web dashboard (MJPEG stream + LiDAR map + waypoints)
...
from web_ui import draw_dashboard, HTML_PAGE
...
# --- Toleance ---
NAV_GOAL_TOLERANCE_M = 0.15        # ban kinh dat dich (m)
NAV_HEADING_TOLERANCE_DEG = 8.0    # sai so heading chap nhan (do)
...
NAV_OMEGA_MIN_MOVE = 0.25          # rad/s toi thieu khi quay (chong tinh)
...
# ===== V7: A* Path Planning =====
ASTAR_RESOLUTION_M   = 0.15   # do phan giai grid (m/o)
ASTAR_INFLATE_M      = 0.22   # tang phong obstacle (m) - tuong duong ban kinh robot
ASTAR_SUBWP_TOL_M    = 0.30   # coi sub-waypoint la dat duoc khi dist < N m
ASTAR_REPLAN_DIST_M  = 1.00   # replan khi robot lech xa hon N m so voi diem thu 2 tren path
ASTAR_REPLAN_PERIOD_S = 10.0  # replan theo chu ky N giay (phong tranh drift)

# ===== V7: Local Obstacle Avoidance =====
OBS_SAFE_DIST_M   = 0.40   # DUNG han khi co vat can trong N m phia truoc
OBS_SLOW_DIST_M   = 0.75   # GIAM TAN khi co vat can trong N m phia truoc
OBS_CONE_HALF_DEG = 22.0   # ban con phat hien phia truoc (do)
OBS_MIN_POINTS    = 3      # so diem toi thieu de coi la vat can that su (loc noise)

# ===== V7: Encoder Fusion =====
# Blend encoder odometry voi ICP estimate de cai thien init-delta cho ICP.
# alpha = ty trong cua ICP estimate (1-alpha = encoder)
ENCODER_ICP_ALPHA   = 0.55  # 0 = chi dung encoder; 1 = chi dung ICP (nhu cu)
ENCODER_TRUST_SCANS = 3     # sau N scan ICP thanh cong lien tiep moi tang weight ICP

# ===== Map: raycast occupancy =====
MAP_RAYCAST_FREE_DECREMENT = 0.35  # voxel duoc laser di qua -> tru obs_count
MAP_RAYCAST_HIT_INCREMENT = 1.0    # voxel co diem hit -> cong obs_count
MAP_OBS_MAX = 20.0                 # gioi han tren cua obs_count (tranh kho dong)
MAP_MIN_OBS_TO_DISPLAY = 2.5       # render: chi ve voxel obs >= nguong
...
class SerialManager:
    ...
    def __init__(self, port, baudrate=115200):
        ...
        self.imu_ok = False   # True khi BNO085 co absolute yaw
...
class PoseEKF:
    ...
    State: x = [px, py, theta]  (world frame, SI units)

    Sensors duoc khai thac:
      - Predict   : encoder vx (forward m/s) + IMU gyro wz (rad/s) -> motion model
      - Update A  : ICP full pose [x, y, theta] -> chinh vi tri + heading
      - Update B  : IMU absolute yaw [theta] tu BNO085 -> NEO heading, KHONG drift

    Uu diem so voi blending thu cong:
      1. Kalman gain tu dong can bac sensor (khong can chinh alpha tay)
      2. ICP noise R duoc scale theo chat luong match (error + overlap)
      3. IMU absolute yaw loai bo toan bo heading drift tich luy cua SLAM

    Tham so noise (co the chinh khi chay thuc te):
      Q_XY      : do khong chac chan cua mo hinh chuyen dong (encoder slip)
      Q_THETA   : do khong chac chan goc (gyro noise)
      R_ICP_POS : ICP position noise co ban (m^2)
      R_ICP_ANG : ICP heading noise co ban (rad^2)
      R_IMU_YAW : IMU absolute yaw noise (rat nho, BNO085 +/-0.5deg rms)
    """

    # Noise defaults -- co the override khi khoi tao hoac tu test
    Q_XY       = 0.02    # m -- process noise position (encoder slip ~2cm/scan)
    Q_THETA    = 0.008   # rad -- process noise heading (~0.5 deg/scan)
    R_ICP_POS  = 0.03    # m -- ICP position noise co ban
    R_ICP_ANG  = 0.015   # rad -- ICP heading noise co ban (~1 deg)
    R_IMU_YAW  = 0.0003  # rad^2 -- BNO085 absolute yaw noise (~1 deg rms = (0.017)^2 ~= 3e-4)

    def __init__(self):
        self.x = np.zeros(3, dtype=np.float64)    # [px, py, theta]
    ...
    def reset(self, pose=(0.0, 0.0, 0.0)):
        ...
        self.P = np.diag([0.01, 0.01, 0.005])   # bat dau voi uncertainty nho
        self.initialized = True

    # ------------------------------------------------------------------
    # Predict step: dung encoder + IMU gyro lam mo hinh chuyen dong
    # ------------------------------------------------------------------
    def predict(self, enc_vx, enc_wz, dt):
        """
        enc_vx: forward velocity trong frame robot (m/s), duong = tien
        enc_wz: angular velocity (rad/s), IMU gyro neu co
        dt    : thoi gian giua hai scan (s)
        """
        if not self.initialized:
            return
        ...
        # Jacobian cua motion model (linearization tai x_k)
        F = np.array([
            [1.0,  0.0, -dx * sin_th],
            [0.0,  1.0,  dx * cos_th],
            [0.0,  0.0,  1.0        ],
    ...
    # ------------------------------------------------------------------
    # Update A: ICP full pose
    # ------------------------------------------------------------------
    def update_icp(self, icp_x, icp_y, icp_theta, icp_error=0.05, icp_overlap=0.8):
        """
        icp_x/y/theta : pose tinh duoc boi ICP (world frame)
        icp_error     : residual ICP error (m), dung de scale R
        icp_overlap   : overlap ratio [0..1], thap -> kem tin
        """
        if not self.initialized:
            return

        # Scale R theo chat luong ICP: error lon hoac overlap thap -> noise lon -> tin it
        r_pos = max(self.R_ICP_POS, icp_error * 0.5) ** 2
        r_ang = max(self.R_ICP_ANG, icp_error * 0.3) ** 2
        if icp_overlap < 0.6:
            penalty = 1.0 + (0.6 - icp_overlap) * 8.0   # max ~5x khi overlap=0
            r_pos *= penalty
            r_ang *= penalty
        ...
        try:
            ...
            return   # skip update neu matrix suy bien (hiem gap)
    ...
    # ------------------------------------------------------------------
    # Update B: IMU absolute yaw (chi theta)
    # ------------------------------------------------------------------
    def update_imu_yaw(self, imu_theta_rad):
        """
        imu_theta_rad: heading tuyet doi tu BNO085 (rad, da wrap [-pi, pi]).
        Chi update theta, KHONG anh huong px/py.
        Day la update quan trong nhat: neo heading khong drift.
        """
        if not self.initialized:
            return
...
class LidarPoseManager:
    ...
    async def _process_scans(self, lidar):
        ...
        consecutive_successes = 0   # V7: dem ICP thanh cong lien tiep (reset khi fail)
        scan_count = 0
        accepted = 0
        rejected = 0
        hard_resets = 0
        ...
        # V7: EKF cho pose fusion (encoder + IMU absolute yaw + ICP)
        ekf = PoseEKF()
        ekf.reset(pose)
        ...
        while self.app_state.running and not lidar.stop_event.is_set():
            ...
            # ===== V7: Doc sensor data tu Arduino (1 lock cho ca scan nay) =====
            ...
            # SerialManager luu measured_vx = -robot_vx_arduino -> dao lai
            # TODO: doi thanh +enc_vx_raw neu LiDAR gap phia sau robot
            enc_vx = -enc_vx_raw
            imu_theta_rad = math.radians(imu_theta_raw) if has_imu_theta else None

            # ===== V7: EKF Predict (encoder + IMU gyro) =====
            ekf.predict(enc_vx, enc_wz, dt_scan)

            # ===== V7: EKF Update B -- IMU absolute yaw (neu co) =====
            # Chay truoc ICP: neo heading truoc khi dung lam init_delta
            if imu_theta_rad is not None:
                ekf.update_imu_yaw(imu_theta_rad)

            # ===== Deskewing voi velocity EKF predict =====
            ekf_pose = ekf.pose
            velocity_per_scan = (
                ekf_pose[0] - pose[0],
                ekf_pose[1] - pose[1],
            ...
            # ===== ICP: dung EKF predict pose lam init_delta =====
            result, fail_reason = kissicp.icp_2d(
                points,
                target_map,
                init_delta=ekf.pose,
            ...
            if result is None:
                ...
                if consecutive_failures >= kissicp.MAX_CONSECUTIVE_FAILURES:
                    ...
                    hard_resets += 1
                    velocity_per_scan = (0.0, 0.0, 0.0)
                    # EKF: giu pose EKF (encoder predict) lam best estimate
                    pose = ekf.pose
                    status = 'RESET'
                else:
                    status = 'ICP_FAIL'
                    # Khi fail: pose = EKF predict (encoder + IMU, khong co ICP)
                    pose = ekf.pose
                self._update_status(
                    status,
                    lidar_scans=scan_count,
            ...
            # ===== V7: EKF Update A -- ICP full pose (chat luong-weighted) =====
            ekf.update_icp(new_pose[0], new_pose[1], new_pose[2],
                           icp_error=float(error), icp_overlap=float(overlap))

            # ===== V7: EKF Update B lan 2 -- IMU yaw sau ICP (tinh chinh lai) =====
            if imu_theta_rad is not None:
                ekf.update_imu_yaw(imu_theta_rad)

            # Pose cuoi = EKF (da fusion encoder + IMU yaw + ICP)
            pose = ekf.pose
            velocity_per_scan = step   # giu step ICP cho deskewing scan tiep theo
            local_map.add_scan(points, pose)
            accepted += 1
...
# ======================== DASHBOARD + HTML (xem web_ui.py) ========================


# HTML_PAGE va cac ham draw_* da chuyen sang web_ui.py
...
class SharedAppState:
    def __init__(self):
        ...
        # V7: A* path state
        self.nav_path_waypoints = []     # list of (x, y) sub-waypoints tu A*
        self.nav_path_idx = 0            # index sub-waypoint dang huong den
        self.nav_path_target_id = None   # id cua final target khi plan duoc tinh
        self.nav_last_plan_t = 0.0       # time.monotonic() cua lan plan gan nhat
        self.stats = {
            'status': 'SEARCHING',
            'ball_detected': False,
        ...
        while app_state.running:
            ...
            if nav_control_active:
                ...
                if lidar_status != 'TRACKING' or lidar_age > NAV_LIDAR_STALE_S:
                    ...
                    # Advance sub-waypoint index if close enough
                    if app_state.nav_path_waypoints and app_state.nav_path_idx < len(app_state.nav_path_waypoints):
                        ...
                        sub_dist = math.hypot(sub_wp[0] - lidar_x, sub_wp[1] - lidar_y)
                        if sub_dist <= ASTAR_SUBWP_TOL_M:
                            app_state.nav_path_idx += 1
                        # Use next sub-waypoint as immediate target
                    ...
                    # Khoang cach den final goal (dung cho ARRIVED check, khong dung sub-waypoint)
                    ...
                    if nav_phase == 'TURN_TO_PATH':
                        ...
                        # Quay den goal_theta. Tinh heading_error theo goal_theta
                    ...
                    if nav_phase == 'ARRIVED':
                        ...
                        vx_target = 0.0
                        vy_target = 0.0
                        # Omega theo P-control + min toc do toi thieu de robot kip xoay
                        omega_raw = NAV_HEADING_KP * heading_error
                        # Apply min speed (chong tinh khi heading_err nho nhung chua trong tolerance)
                        ...
                        # Omega trong DRIVE: chi sua nhe heading (line-following)
                        omega_target = float(np.clip(
                            NAV_HEADING_KP * heading_error * 0.7,  # giam Kp khi dang chay
                        ...
                        if vx_target > 0.01:
                            ...
                            if len(live_cloud) >= OBS_MIN_POINTS:
                                cloud_world = np.array([[p[0], p[1]] for p in live_cloud], dtype=np.float32)
                                ...
                                n_safe = int(np.sum(mask_safe))
                                if n_safe >= OBS_MIN_POINTS:
                                    # Vat can qua gan: dung khan cap
                                    vx_target = 0.0
                                    omega_target = 0.0
