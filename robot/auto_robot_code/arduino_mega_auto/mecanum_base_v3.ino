...
float WHEEL_KV_PWM_PER_RPM = 255.0f / RPM_MAX;  // runtime-tunable via KV command
float WHEEL_KS_PWM = 22.0f;                      // runtime-tunable via KV command
...
void updateOdometry(float dt) {
  ...
  robot_wz_rad_s = wz;
  robot_wz_enc_rad_s = wz_encoder;  // always wheel-based, regardless of IMU
}
...
void processCommand(const char* cmd) {
  if (cmd[0] == 'V' && cmd[1] == ' ') {
    ...
    // IMU_OFFSET <target_yaw_rad>
    // Set offset sao cho imu_yaw_rad output = target_yaw_rad ngay luc nay
    float target = (float)strtod(cmd + 11, nullptr);
    if (imu_has_yaw) {
      imu_yaw_offset_rad = imu_yaw_raw_rad - target;
      imu_yaw_rad = wrapAngle(imu_yaw_raw_rad - imu_yaw_offset_rad);
      headingLocked = false;  // reset heading hold voi yaw moi
      Serial.print("[IMU] Offset updated: raw=");
      Serial.print(imu_yaw_raw_rad * 180.0 / PI, 2);
      Serial.print(" target=");
    ...
    long mode = strtol(cmd + 9, nullptr, 10);
    externalHeadingControl = (mode != 0);
    headingLocked = false;
    headingIntegral = 0.0f;
    Serial.print("[HEADCTRL] ");
    Serial.println(externalHeadingControl ? "EXTERNAL" : "INTERNAL");
  }
}
...
void loop() {
  ...
  if (++fbCount >= 10) {
    ...
    Serial.print(imu_has_yaw ? 1 : 0); Serial.print(' ');
